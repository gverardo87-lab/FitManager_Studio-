...
│   ├── contracts.py     CRUD contratti + batch fetch enriched + renew + incassa-residuo (G6) + terminate/settlement-preview/reopen (G7.3-G7.4, bilaterale ADR-018)
...
**Già in codice (Blocchi 0-4 + Prereq P + terminazione G7.0→G7.9):**
...
8. **Credit guard** (`create_event`): se `id_contratto` esplicito e `crediti_usati >= crediti_totali`
   ...
   **Occupazione-credito (G7.8/ADR-017):** `crediti_usati = COUNT(PT in stato Programmato|Completato)` —
   `Rinviato` e `Cancellato` **NON** occupano (il rinvio libera il credito → riprenotabile). **MAI**
   riusare `!= 'Cancellato'` sui siti credito (regredirebbe G7.8); i siti calendario/recency lo tengono.
