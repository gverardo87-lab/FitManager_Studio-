 ...
 *
 * Documento medico-sportivo proprietario FitManager Studio+.
...
function createCoverSheet(
  ...
  // Brand
  const brandRow = ws.addRow(["FitManager Studio+"]);
  ...
  // Footer
  const footerRow = ws.addRow(["Documento generato da FitManager Studio+"]);
...
export async function exportWorkoutExcel({
  ...
  for (const session of sessioni) {
    ...
    // ── Footer ──
    ws.addRow([]);
    const footerCells = ["FitManager Studio+", "", "", "", "", "", "", new Date().toLocaleDateString("it-IT")];
