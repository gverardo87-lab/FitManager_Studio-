...
const JOURNEY_PHASES: JourneyPhase[] = [
  ...
  },
  {
    ...
    getAction: (p) => ({ type: "link", href: `/clienti/${p.clientId}/misurazioni?from=clienti-${p.clientId}` }),
  },
  {
    key: "scheda",
    label: "Scheda",
...
const QUICK_ACCESS = [
  ...
  {
    key: "misurazioni",
    href: (id: number) => `/clienti/${id}/misurazioni?from=clienti-${id}`,
    icon: TrendingUp,
    label: "Misurazioni",
    description: "Registra e consulta misurazioni fisiche",
...
export function PanoramicaTab(props: PanoramicaTabProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Sezione 3: Comunicazioni (link al registro) ── */}
      <CommunicationLink clientId={clientId} />
...
// ════════════════════════════════════════════════════════════
// COMMUNICATION LINK — rimando compatto al registro
// ════════════════════════════════════════════════════════════

function CommunicationLink({ clientId }: { clientId: number }) {
  ...
  return (
    <Link href={`/comunicazioni?tab=registro&cliente=${clientId}`}>
      <div className="flex items-center gap-3 rounded-xl border px-4 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md hover:border-emerald-200 dark:hover:border-emerald-800">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/30">
          <MessageCircle className="h-4.5 w-4.5 text-emerald-600 dark:text-emerald-400" />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold">
            {count} {count === 1 ? "comunicazione" : "comunicazioni"}
          </p>
          <p className="text-xs text-muted-foreground">Vedi registro completo</p>
        </div>
