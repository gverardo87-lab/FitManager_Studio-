...
- **Data pagamento default**: oggi — descrive quando il denaro entra, non quando la rata era dovuta. DatePicker sempre visibile per registrazioni tardive o correzione manuale.
...
| Link kiosk da localhost non raggiungibile da altri device | `window.location.origin = "http://localhost:3000"` → URL nel link non raggiungibile su rete esterna | Mostrare warning amber nel dialog quando `hostname === "localhost"`. I link da condividere usano la `public_base_url` FRP gestita dalla licenza; l'IP LAN resta solo un accesso locale, non l'origine pubblica. |
...
| **Safety map cache stale dopo modifica anamnesi (CRITICO DEMO)** | `useUpdateAnamnesi()` invalidava `clients` e `dashboard` ma NON `exercise-safety-map`. Con `staleTime: 5min`, la cache serviva `condition_count: 0` anche dopo aggiunta di patologie/infortuni → profilo clinico invisibile nel builder. Crash 500 di `plan-package` mascherato da errore CORS | `useUpdateAnamnesi` e `useUpdateClient` DEVONO invalidare `["exercise-safety-map", client.id]`. **Regola ferrea**: ogni mutation che tocca anamnesi invalida anche la safety map. Incidente P0: `docs/incidents/INC-2026-03-28-safety-engine-blind-spot.md` |
| **BuilderSafetyCard nascosta dietro toggle (CRITICO DEMO)** | `lg:hidden` + `showAdvanced` gate nascondevano il profilo clinico su desktop. Il SciencePanel (che doveva sostituirlo) era dietro un toggle Flask default OFF → zero informazioni cliniche visibili | BuilderSafetyCard SEMPRE visibile quando `condition_count > 0`. Zero condizioni responsive, zero toggle, zero tab. Informazioni cliniche = non negoziabili |
| **Crash 500 mascherato da errore CORS** | Quando FastAPI crasha prima del middleware CORS, il browser mostra `No Access-Control-Allow-Origin` / `ERR_FAILED` invece del vero errore 500. Debugging fuorviante | Se errore CORS su endpoint che normalmente funziona → guardare PRIMA stderr/log uvicorn per crash 500. L'errore CORS e' sintomo, non causa |
