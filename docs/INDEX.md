...
| `SPEC_G9.7_SEMANTICA_PER_CLASSE.md` | 🟡 **APERTA — DA IMPLEMENTARE** (ratifica 2026-07-07) | Generalizzazione per-classe delle leggi semantiche (ADR-024): matrice assi×regole → mai-silenzio write-path eventi → recupero esplicito orfani → occupazione mai nuda a video → guard di classe + perimetro transizioni → birth-auditor + Hypothesis estesa. AC = 13 finding di `operations/AUDIT_CREDITI_EVENTI_ORFANI_2026-07-07.md` |
...
| `AUDIT_PRE_RELEASE_2026-07-16.md` | **Triade auditor read-only sul batch di release v1.0.13→HEAD (206 commit, candidata v1.0.14)** — step 1 del runbook: financial-invariant-verifier **MONEY AXIS PRESERVED** (394 oracle verdi, 1 LOW dead-code F1) · docs-drift 8 finding tutti docs-layer (2 HIGH su INDEX stesso, 1 falso positivo confutato) · semantic-birth-auditor (prima corsa) **AT RISK**: M1 reale (`CONSUNZIONE` buca il guard H1 di unpay — prod log-only) + 2 re-inline dell'autore. Definisce la **fetta R1** pre-bump (5 fix + gemelli, nessun nuovo ADR) + deferral (M3→P1 in SPEC_P). Gate: taglio sbloccato DOPO R1 |
