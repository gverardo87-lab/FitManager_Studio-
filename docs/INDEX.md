...
| File | Stato | Scopo |
|------|-------|-------|
| `SPEC_R0_PROTEZIONE_RELEASE_V1_0_15.md` | 🟠 **APERTA — R0.1 + R0.1.5-core + R0.2 ✅; R0.1.5-live HOLD; R0.3 prossimo** | R0.2 rende single-DB le procedure vive: un solo `upgrade head` sul `DATABASE_URL` configurato, legacy `crm_dev.db` rifiutato prima di Alembic e rehearsal senza requisito dev; canary/schema 22/22 e full backend 910/910. Apply edge + chain pubblica + probe strict restano interlock obbligatorio prima di R0.4/P |
