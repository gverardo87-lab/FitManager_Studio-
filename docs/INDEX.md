...
| `IMPL_PLAN_FINANCIAL_REALIGN.md` | **Unico piano attivo + RESUME POINT del filone finanziario** (workflow-verified). FATTI: Blocchi 0-3 + Prereq P + PREREQ-prezzo + Giro 1 vocabolario + data_scadenza-null + **G6 (incassa residuo)**. **PROSSIMO: convergenza residuo (SPEC_REVISIONE_PRE_G7) → G7 (terminazione) → G1 (cifratura).** Stato nel banner in testa al doc |
| `SPEC_REVISIONE_PRE_G7.md` | **Spec di revisione pre-G7 (bridge Chat→Code, vincolante sugli AC).** Sez. A — convergenza del residuo a `contract_state.residuo()` (refactoring output-invariante, **prerequisito di G7**: protegge anche i guard di pagamento `rates.py`); Sez. B — copertura SOSPESO nel workspace `renewals_cash` (cambiamento funzionale, indipendente). **Verificata sul codice 2026-06-23**: A.4 estesa a 5 siti mancati (`rates.py:525/734`, `dashboard.py:497/446`, `DeleteContractDialog.tsx:58`). A e B = commit separati (§1.1). Da implementare |

> **Spec finanziarie implementate → `docs/archive/specs/`.** Le spec-feature SPEC_RINNOVO, SPEC_GESTIONE_FINANZIARIA_TEMPORALE e SPEC_RINNOVI_SCADUTI_E_RETENTION sono state implementate e il loro modello vive ora nel SSoT (FDM + TASSONOMIA + `contract_state.py`). Sono archiviate come design-record insieme ai rispettivi IMPL_PLAN. Il dominio finanziario vivo è: FDM + TASSONOMIA (SSoT) · SPEC_VOCABOLARIO (Giro 2 aperto) · SPEC_REVISIONE_PRE_G7 (convergenza residuo + SOSPESO workspace, da implementare) · IMPL_PLAN_FINANCIAL_REALIGN (piano attivo) · ADR-014/015 (decisioni).
