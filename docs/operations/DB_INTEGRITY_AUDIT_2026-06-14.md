...
> - ⚠️ **P1 — DROP tabelle catalog stale da crm.db: 9/10 FATTE** (sera 2026-06-14). Droppate le 9 catalog **pure** (esercizi, esercizi_media, esercizi_muscoli/articolazioni/condizioni, esercizi_relazioni, muscoli, articolazioni, condizioni_mediche): zero FK business in entrata. crm.db 38→29 tabelle; `foreign_key_check` pulito; conteggi business invariati; integrity OK. **VACUUM: 4.56 MB → 0.91 MB (−80%)** — conferma il gonfiore da detrito. Backup `crm.db.bak-threadA-p1`. I 3 orfani scartati (644/877/905) ora non risolvono più nella tabella stale (rimossa) → refs dangling in schede TEST (innocue).
> - ✅ **P1-bis — `metriche` (10ª tabella) RISOLTA + BUG LATENTE CORRETTO (sera 2026-06-14).** Indagine sul backup reale di Chiara (8 giu) + crm.db fresh ricostruito: **bug latente confermato e riprodotto** — un crm.db *fresh* (installer, che non spedisce crm.db → `create_db_and_tables` esclude metriche) con `foreign_keys=ON` crasha `no such table: metriche` al primo salvataggio misurazione/obiettivo. Chiara/Alessio NON colpiti (monolite via Alembic con metriche); **a rischio le nuove installazioni**. Fix: (1) modelli `goal.py`/`measurement.py` → `id_metrica` cross-DB senza FK; (2) `schema_sync._fix_cross_db_fk` esteso (`_CROSS_DB_FK_FIXES`) ricrea valori_misurazione/obiettivi_cliente senza FK preservando le righe — auto-ripara i DB deployati al boot; (3) **bonus** `_drop_stale_catalog_tables` droppa anche le catalog stale POPOLATE dai DB deployati (Chiara: 37→28 tabelle, −80% size) + VACUUM. Validato su copia del backup Chiara (145 valori preservati, FK rimosse, metriche+9 stale droppate, integrity OK, idempotente). Suite 362 + 7 nuovi test, ruff clean. Dev crm.db guarito: **ADR-003 chiuso al 100%** (zero catalog in crm.db). Pitfall #15. Ship nella **prossima versione** (auto-heal Alessio/Chiara al primo boot).
> - ✅ **P3 ESEGUITO (sera 2026-06-14).** P3a: `crm_dev.db` archiviato (`→ .archived-20260614`; il runtime usa solo la stringa `"crm_dev" in DATABASE_URL` per il label DEV/PROD, non apre il file). P3b: i **7 nomi duplicati** erano 1 attivo canonico (junction complete) + 1 inattivo degradato (zero junction) → eliminati i 7 inattivi (351/468/990/552/1094/389/349) da catalog.db con pulizia completa (media/junction/relazioni), + remap dei 3 ref schede dev al gemello attivo (351→350, 990→621, 552→169). catalog 529→**522**, attivi 466 invariati, **0 nomi duplicati**, integrity OK, seed aggiornato (522 esercizi / 738 media), sha256 ricalcolato. Backup `*.bak-threadA-p3`.
>
> **🏁 THREAD A CHIUSO.** Audit→remediation completa: orfani risolti (Ondate 1+2), ADR-003 chiuso al 100% + bug latente metriche corretto (P1/P1-bis), housekeeping (P3). Resta solo l'azione di **release** (ship del fix metriche nella prossima versione).
