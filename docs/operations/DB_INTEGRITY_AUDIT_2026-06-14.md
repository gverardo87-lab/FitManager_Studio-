...
> - ⬜ **P1-bis — `metriche` (10ª tabella) NON droppabile come le altre.** Landmine scoperto: `obiettivi_cliente.id_metrica` e `valori_misurazione.id_metrica` hanno **FK locale reale** a `metriche.id` (modelli `goal.py`/`measurement.py`: `foreign_key="metriche.id"`), con 146 righe business che la usano. crm.db.metriche == catalog.db.metriche (22 righe identiche); tutti gli id_metrica usati esistono in catalog.db. Droppare romperebbe l'enforcement FK sulle scritture future. **Task coordinato dedicato:** (1) migrare i modelli al pattern cross-DB (id_metrica = int semplice, come `componenti_pasto.alimento_id`), (2) Alembic migration per rimuovere la FK (ricrea le 2 tabelle), (3) DROP metriche, (4) test goal/measurement. Verificare anche se un crm.db *fresh* (che esclude metriche) rompe già oggi su insert misurazioni con FK a tabella assente.
