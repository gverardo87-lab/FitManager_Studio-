...
**Gate di consolidamento:** la decisione è presa; **commit della v2.2 e invio della mail al fornitore avvengono dopo l'autotest** (`docs/learning/LEARNING_MEDIA_CLOUD_ARCHITECTURE.md` §9). Correzioni emerse dall'autotest riplasmano questo delta prima del commit.

**Aggiornamento 2026-06-13 (verifica dall'interno, Claude Code):** la verifica del vincolo `catalog.db` read-only/cifrato ha riplasmato **OP-5** (caching + claim "zero sync") e **OP-8.4** (`media.ts` vieta gli URL assoluti) *prima* dell'autotest. Decisione POC: **R1** (URL coniati al build-time, update libreria col ciclo di release); **R2** (manifest per update live) rimandato a v2. Correzioni speculari applicate a LEARNING §0/§5/§9.
...
4. **`media.ts` — il vero blocco (verificato dall'interno, Claude Code 2026-06-13):** `getMediaUrl()` in `frontend/src/lib/media.ts` ritorna **path relativi** e **vieta esplicitamente gli URL assoluti** (commento nel file: falliscono cross-porta/via tunnel; oggi i media sono *same-origin* via Next rewrite `/media/* → backend`, e raggiungono l'atleta **attraverso il tunnel** — è proprio il problema di banda che la v2.2 risolve). Non basta che `ExerciseMedia.url` regga URL assoluti (regge, è `str` libero, TEXT SQLite): è lo **strato di risoluzione frontend** ad assumere il contrario. Va evoluto perché il routing-per-classificazione funzioni: **media generici → URL assoluto al media host (cross-origin); dato personale → relativo/same-origin via tunnel.** Punto d'intervento: `frontend/src/lib/media.ts` + i consumer dei tag `<video>`/`<img>`.
