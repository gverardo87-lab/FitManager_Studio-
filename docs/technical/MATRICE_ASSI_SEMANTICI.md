...
| **Stati crediti/wallet (APERTO/SALDATO/ANNULLATO)** | ✅ costanti `STATO_CREDITO_*` (G9.4-bis.0) | ✅ `residuo_credito` SSoT | ⚠️ **casa P4/P5**: Portafoglio fail-loud sulle azioni disponibili | ✅ G9.7.4: `test_g974_stati_credito_no_reinline` (classificazioni solo via `STATO_CREDITO_*`) | ⚠️ worklist ok, breakdown minimale → **P4/P5** | ⚠️ composizione wallet×insoluto/compensazione → **P4/P5** (Portafoglio) |
