...
- **Dove (code-grounded, verificare sul vivo):**
  - Helper backend `[Bridge ratify 2026-06-24]`: il conteggio Programmati è una proiezione di `Event` come `_count_sedute_erogate`. **Strada preferita = additiva e sicura:** aggiungere il SOLO conteggio `Programmato` come query separata, **senza toccare `_count_sedute_erogate`/`_settlement_for`** — il path del conguaglio (G7.3, testato e verde) resta intatto; non si rifattorizza codice verde dentro un commit functional-change (regola "non mischiare refactor e functional change", e D2/D4 è già functional). La query extra su un dry-run singolo-contratto è trascurabile. **Alternativa più elegante ma scartata di default:** un solo `select(Event.stato, func.count).group_by(Event.stato)` come `get_contract` (Completato→conguaglio, Programmato→display, una query non due) — ma rifattorizza `_settlement_for` (codice verde) → se scelta, dev'essere **output-invariante** su `sedute_erogate` e verificata contro i test G7.3 come oracolo. Entrambe rispettano SSoT-`Event` e il confine display-only.
