...
**Conseguenza operativa:** da ora i commit ordinari su `FitManager_Studio` **non** riallineano `main` — `main` resta fermo all'ultima release verificata. Niente più "balletto" a ogni commit; l'allineamento avviene solo come step finale di `build-release.sh` (dopo VERIFY), su una release confermata.
